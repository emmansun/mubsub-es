    ...
    this.latest(
      ...
      this.handle(true, function (latest, collection) {
        if (self.transport === 'polling') {
          self.startPollingListener(latest)
          return
        }
          ...
          if (!doc) {
            return setTimeout(function () {
              ...
              if (self.options.recreate) {
                if (self.options.mode === 'polling') {
                  self.createPolling().startPollingListener(latest)
                } else if (self.options.mode === 'auto') {
                  self.createCapped().startTailableListener(latest, true)
                } else {
                  self.createCapped().startTailableListener(latest)
                }
              }
            }, 1000)
          }
    ...
    this.latest(
      ...
      this.handle(true, function (cursor, collection) {
        ...
        const poll = function () {
          ...
          const query = cursor && cursor._id ? { _id: { $gt: cursor._id } } : {}
      ...
      cursor
        ...
        .then((doc) => {
          ...
          if (doc) { // further check if there are larger _id than this doc.
            ...
            callback(undefined, null, collection)
          } else {
            collection
